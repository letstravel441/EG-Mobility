 ...
 */
class oilchangeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view=inflater.inflate(R.layout.fragment_oilchange, container, false)
        var recyclerView = view.findViewById(R.id.stations4) as RecyclerView
            ...
            override fun onDataChange(p0: DataSnapshot) {
                p0.children.forEach{
                    ...
                    if(f.child("non-ev-car").exists()) {
                        var k=0;
                        while(f.child("non-ev-car").child("non-ev-car-services$k").exists()) {
                            var b =
                                f.child("non-ev-car").child("non-ev-car-services$k").value.toString()
                            if (b == "oil and filter change") {
                                val u = it.getValue(Busi_user::class.java)
                                if (u != null) {
                                    adapter.add(UserItem4(u))
                                }
                            }
                            k++
                        }
...
class UserItem4(val u:Busi_user): Item<GroupieViewHolder>(){
    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        ...
        viewHolder.itemView.setOnClickListener(View.OnClickListener { v ->
            ...
            intent.putExtra("linkservice","oilchange")
