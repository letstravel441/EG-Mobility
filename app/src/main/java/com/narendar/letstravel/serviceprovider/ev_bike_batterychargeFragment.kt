 ...
 */
class ev_bike_batterychargeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view=inflater.inflate(R.layout.fragment_ev_bike_batterycharge, container, false)
        var recyclerView = view.findViewById(R.id.stations13) as RecyclerView
        ...
        ref.addListenerForSingleValueEvent(object: ValueEventListener {
            override fun onDataChange(p0: DataSnapshot) {
                p0.children.forEach{
                    var f=it
                    if(f.child("ev-bike").exists()) {
                        var k=0;
                        while(f.child("ev-bike").child("ev-bike-services$k").exists()) {
                            var b =
                                f.child("ev-bike").child("ev-bike-services$k").value.toString()
                            if (b == "Battery charging") {
                                val u = it.getValue(Busi_user::class.java)
                                if (u != null) {
                                    adapter.add(UserItem13(u))
                                }
                            }
                            k++
                        }
...
class UserItem13(val u:Busi_user): Item<GroupieViewHolder>(){
    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        ...
        viewHolder.itemView.setOnClickListener(View.OnClickListener { v ->
            ...
            intent.putExtra("type_service","ev-bike")
            intent.putExtra("linkservice","Battery charging")
